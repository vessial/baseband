...
![image-20210308164203970](C:\project\ec20\code\image-20210308164203970.png)
...
![image-20210322162235900](C:\Users\vessial\AppData\Roaming\Typora\typora-user-images\image-20210322162235900.png)
...
![image-20210325161755813](C:\Users\vessial\AppData\Roaming\Typora\typora-user-images\image-20210325161755813.png)
...
DL_DCCH的Information Transfer字段里面包含了来自MME发送过来的认证请求数据(LTE NAS EMM信令消息id 0x52)，包含有nas key set id, 16个字节的认证随机数据auth_param rand，以及16个字节的auth param AUTN数据，SIM卡通过收到的这两个关键信息进行认证，并计算生成Auth_resp发送给MME进行比较完成本地端和服务器端的认证，本直端计算如下。
本地端收到的rand(16bytes)+AUTN(16bytes)
K为sim卡和MME都持有的sim卡的唯一隐私数据，sim卡端只有sim卡芯片可以读取

SIM卡端密钥派生过程，f1/2/3/4/5为sim卡的计算功能函数
AK=f5(K,rand)  
IK=f4(K,rand)
CK=f3(K,rand)
